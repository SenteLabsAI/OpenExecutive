...
async def _apply_state_from_source(source_dir: Path, settings: Any) -> dict[str, Any]:
    ...
    # Back to the defaults, then the source's optional workspace.yaml. Like
    # the wipes below this is unconditional: a fixture without the file must
    # not inherit the previous company's mode or zone. No scheduler side
    # effects — scheduled_actions were just rebuilt from memory.json.
    workspace_applied = _apply_workspace_file(source_dir / "workspace.yaml")
...
def _apply_workspace_file(workspace_path: Path) -> dict[str, Any]:
    """Reset the workspace settings, then apply an optional ``workspace.yaml``
    (``mode: solo|team``, ``timezone: <IANA zone>``). Returns what is in effect.

    A value that does not validate is logged and skipped rather than failing
    the load half-way (the profile and docs are already swapped by now).
    """
    import yaml
